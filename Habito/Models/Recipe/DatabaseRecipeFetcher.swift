...
class DatabaseRecipeFetcher {
    ...
    func fetchAllRecipes() throws -> [Recipe] {
        ...
        while(sqlite3_step(recipeStatement) == SQLITE_ROW) {
            ...
            let suggestions = String(cString: sqlite3_column_text(recipeStatement, 2))
            let timeInMinute = Int(sqlite3_column_int(recipeStatement, 3))
            let calories = Int(sqlite3_column_int(recipeStatement, 4))
            let ratings = Int(sqlite3_column_int(recipeStatement, 5))
            
            
            let fetchedRecipe = Recipe(name: name, ingredients: ingredients, suggestions: suggestions, timeInMinutes: timeInMinute, calories: calories, rating: ratings)
